...
class CQFSHSVDTrainer:
    N_CASES = 50
    N_RAN_STARTS = 15

    STATISTICS_FILE = "S_CQFSHSVD_statistics"
    ...
    def __get_experiment_ids(self, ps, alphas, ranks, degs, parameter_product=True):

        if parameter_product:
            expIDs = [get_experiment_id(x[0], x[1], x[2], x[3]) for x in
                      itertools.product(alphas, ranks, degs, ps)]
        else:
            expIDs = [get_experiment_id(x[0], x[1], x[2], x[3]) for x in zip(alphas, ranks, degs, ps)]

        return expIDs

    def train_many(self, ps, alphas, ranks, degs, parameter_product=True, cpu_count_div=2,
                   cpu_count_sub=0):

        expIDs = self.__get_experiment_ids(ps, alphas, ranks, degs,
    ...
    def compute_statistics(self, S_CF, S_CBF, ps, alphas, ranks, degs, parameter_product=True):
...
def _print_train_failed(expID):
    __print(f"{expID} It is not possible to train without the proper selection.\n"
            f"{expID} Please run the selection procedure through CQFSHSVD first.")
